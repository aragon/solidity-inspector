...
class ContractStructure {
  ...
  toJSON(mergeWithParents) {
    ...
    for (let part of ['constructor', 'functions', 'events', 'contstantFunctions'])
      for (let itemName in structure[part])
        structure[part][itemName] = typeof(structure[part][itemName].toJSON) == 'function' ?
          structure[part][itemName].toJSON() : {};
